##### Jan 13 2025 ######
###########
## same as Large_Fit.R but now updating the history (ave_prop)
## current history of a player now only includes games that are played within the same session
##  the max games to look back is prev_n, elsewise it's the current number of games in the session
...
#the number of games to consider for recent performance
prev_n = 1

tidy_games <- map_dfr(users, get_hist, mid_1000_games, prev_n = prev_n) %>% 
  as_tibble()

#adding history
#### TEST: history only includes games played in the same session - with recent games being weighted the highest 
init_data = tidy_games %>%
  ...
  group_by(focal_user) %>%
  mutate(time_diff = UTCDateTime - lag(UTCDateTime)) %>% #time difference between games for each player
  mutate(curr_session_length = !((time_diff > 300) | is.na(time_diff))) %>% #FALSE is start of a session, #TRUE is mid session
  mutate(curr_session_length = (ave(curr_session_length, cumsum(!curr_session_length), FUN = cumsum)) + 1) %>% #the length of the session at the current game - again use def as <300 sec played in the same session
  mutate(hist_to_use = ifelse(curr_session_length >= prev_n + 1,  #the number of games to consider for history 
                              prev_n + 1, #prev_n
                              curr_session_length) - 1) %>% #current number of games in session
  mutate(curr_history = map_dbl(row_number(), ~ {
    i <- .x
    window_size <- hist_to_use[i]
    mean(focal_result[(i - window_size):(i - 1)])
    })) %>% 
  mutate(curr_history = ifelse(hist_to_use == 0, #if starting first game in session (no history to use) set their recent performance to their historical avg
                               mean(focal_result), #mean historical performance
                               curr_history)) %>% #avg performance in past n games - mean historical performance
  mutate(ave_prop = curr_history - mean(focal_result)) %>%
  filter(focal_result != 0.5) %>% #filter out the draws
  dplyr::select(focal_user, focal_id, focal_white, focal_win_prop, elo_diff, focal_result, time_diff, curr_history, ave_prop)
...
stan_file <- "/home/adamgee/scratch/code/final_model_scale_priors_gen.stan"
...
fit3_ave$save_object(file = "/home/adamgee/scratch/results/GM_middle_1000_game_gen.RDS")
